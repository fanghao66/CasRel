...
import os
from token_func import rematch, create_dir
...
create_dir(path['output_dir'])  # 创建输出相关文件夹
create_dir(path['dev_result_path'])  # 创建输出相关文件夹
create_dir(path['result_path'])  # 创建输出相关文件夹
...
def extract_spoes(text):
    ...
    token_ids_1 = token_ids  # [T]
    segment_ids_1 = segment_ids
    token_ids = torch.tensor([token_ids]).to(device)  # [1,T]
    segment_ids = torch.tensor([segment_ids]).to(device)

    # 获取每个token是否属于主体的开头和结尾的概率平方值, [N*T,2]
    subject_preds = sub_model(input_ids=token_ids, token_type_ids=segment_ids,
                              batch_size=1,
                              sub_train=True)

    # 抽取subject 得到预测的主体span片段
    subject_preds = subject_preds.view(1, -1, 2)  # [N*T,2] -> [N,T,2]
    ...
    if subjects:
        spoes = []
        token_ids = np.repeat([token_ids_1], len(subjects), 0)  # [1,T] -> [S,T]
        segment_ids = np.repeat([segment_ids_1], len(subjects), 0)  # [1,T] -> [S,T]
        segment_ids = torch.tensor(segment_ids).to(device)
        token_ids = torch.tensor(token_ids).to(device)
        subjects_len = len(subjects)
        subjects = torch.tensor(subjects).to(device)  # 第一阶段预测出来的主体范围
        # 传入subject，抽取object和predicate
        subject_preds, object_preds = sub_model(
            input_ids=token_ids, token_type_ids=segment_ids,
            subject_ids=subjects, batch_size=subjects_len,
            sub_train=True, obj_train=True
        )
        object_preds = object_preds.detach().cpu().numpy()  # [S,T,num_relations,2]
...
class data_generator:
    ...
    def data_res(self):
        ...
        for i in indices:
            ...
            token_ids = token['input_ids']  # 整个文本对应的token id
            segment_ids = token['token_type_ids']
            attention_mask = token['attention_mask']
            # 整理三元组 {s: [(o, p)]}
            spoes = {}
            for s, p, o in d['spo_list']:
                s = tokenizer.encode_plus(s)['input_ids'][1:-1]  # 主体对应的token id
                p = predicate2id[p]  # 关系对应的类别id
                o = tokenizer.encode_plus(o)['input_ids'][1:-1]  # 客体对应的token id
                s_idx = search(s, token_ids)
                o_idx = search(o, token_ids)
                if s_idx != -1 and o_idx != -1:
                    s = (s_idx, s_idx + len(s) - 1)  # 主体对应的token index范围
                    o = (o_idx, o_idx + len(o) - 1, p)  # 客体对应的token index范围 + 关系类别id
                    if s not in spoes:
                        spoes[s] = []  # 将同一个主体的信息用一个列表表示
                    spoes[s].append(o)  #
            if spoes:
                # subject标签
        ...
        batch_token_ids = sequence_padding(batch_token_ids)  # [N,T]
        batch_segment_ids = sequence_padding(batch_segment_ids)  # [N,T]
        batch_subject_labels = sequence_padding(
            batch_subject_labels, padding=np.zeros(2)
        )  # [N,T,2]
        batch_subject_ids = np.array(batch_subject_ids)  # [N,2] 每个关系对应的主体范围
...
sub_model = REModel_sbuject_2.from_pretrained(
    ...
    num_relations=len(id2predicate)  # 关系类别数目
)
no_cuda = False
...
param_optimizer = list([(n, p) for n, p in sub_model.named_parameters() if p.requires_grad])
...
for epoch in range(epochs):
    for setp, loader_res in enumerate(loader_train):
        # scheduler = WarmupLinearSchedule(optimizer, warmup_steps=warmup_steps,
        #                                  t_total=train_steps)  # warmup can su
        batch_token_ids = loader_res['batch_token_ids'].to(device)  # [N,T]
        ...
        # N:表示批次样本数目，也就是一个批次中，样本的数目是多少
        # T:表示样本的token数量，也就是一个样本有多少个token
        # num_relation:关系类别的数目
        sub_out, obj_out = sub_model(
            input_ids=batch_token_ids,  # [N,T] N个文本，每个文本有T个token id, bert本身输入
            token_type_ids=batch_segment_ids,  # [N,T] bert本身输入, 对应的type类别id
            attention_mask=batch_attention_mask,  # [N,T] bert本身输入，对应的各个文本的token是否是填充的，填充位置为0
            labels=batch_subject_labels,  # [N,T,2] 实体识别分支的实际值 N个文本有T个token，每个token是否是主体的开始或者结尾
            subject_ids=batch_subject_ids,  # [N,2] 针对每个文本，选择一个主体进入关系识别分支
            batch_size=batch_token_ids.size()[0],  # 批次大小，就是N值
            obj_labels=batch_object_labels,  # [N,T,num_relation,2] N个文本有T个token，每个token属于各个关系类别的开头、结尾；
            sub_train=True,
            obj_train=True
        )
    ...
    if f1 > best_acc:
        ...
        model_to_save.save_pretrained(output_dir, safe_serialization=False)  # 保存模型对象(利用transformers的API)
        tokenizer.save_pretrained(output_dir)  # token模型保存(利用transformers的API)
